...
async function showUpdatePositions(team_id) {
    try {
        const position = await db.position
        .where('team_id')
        .equals(team_id)
        ...
        const select = document.getElementById("update_pos");
        if (Object.keys(position).length == 0){
            select.innerHTML = '<option value="player" id="player">Player</option>';
        }
        else{
...
async function showDeletePositions(team_id) {
    try {
        const position = await db.position
        .where('team_id')
        .equals(team_id)
        ...
        const select = document.getElementById("delete_pos");
        if (Object.keys(position).length == 0){
            select.innerHTML = '<option value="player" id="player">Player</option>';
        }
        else{
